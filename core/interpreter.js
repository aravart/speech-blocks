...
SpeechBlocks.Interpreter.prototype.moveBlock = function(command) {
  ...
  if (this.isBlockIdValid(command.block)) {
    if (command.where == 'trash') { this.deleteBlock(command.block); return; }
    else if (command.where.block == null || !this.isBlockIdValid(command.where.block.toString())) { return; }
    ...
    switch (command.where.position) {
      ...
      case 'lhs': case 'rhs': case 'to the right of': var inputList_ = this.controller_.getBlockValueInputs(command.where.block);
      ...
      else { this.controller_.moveBlock(command.block, new SpeechBlocks.ValueInput(command.where.block, inputList_[0])); } break;
      case 'inside': var statementList_ = this.controller_.getBlockStatementInputs(command.where.block);
      if (statementList_.length < 1) { console.log('NO STATEMENT INPUTS FOR SPECIFIED BLOCK'); }
      else { this.controller_.moveBlock(command.block, new SpeechBlocks.StatementInput(command.where.block, statementList_[statementList_.length-1])); }break;
    }
  }
};
